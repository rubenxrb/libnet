...
int main(int ac, char *av[])
{
	...
	echoServAddr.sin_port = htons(echoServPort);		/* server port 7 echo */
	...
	if (send (sock, echoString, echoStringLen, 0) != echoStringLen)
		exitWithError("send() sent a different number of bytes that expected!");
	...
	while (totalBytesRcvd < echoStringLen)
	{
		...
		if ((bytesRcvd = recv(sock, echoBuffer, RCVBUFFERSIZE - 1, 0)) <= 0)
			exitWithError("recv() failed or connection closed prematurely");
		totalBytesRcvd += bytesRcvd;	/*keep tally of total bytes */
		echoBuffer[bytesRcvd] = 0;		/*terminate string */
		puts(echoBuffer);				/* print echo buffer */
	}
